...
// \return						0
 int choix(const char* cmd, GRAPHE graphe){
	...
	//Permet de lister les stations et en fonction de l'argument
	//affiche ou par ordre alphabétique ou selon les lignes
	if(cmd[0] == 'l' && cmd[1] == 's'){
		//Ordre alphabétique
		if(cmd[2] == '-' && cmd[3] == 'a'){
	...
	//Affiche toutes les liaisons entre toutes les stations
	...
	//Permet d'afficher le nombre de station
	if(cmd[0] == 'n' && cmd[1] == 'o' && cmd[2] == 'm' && cmd[3] == 'b' && cmd[4] == 'r' && cmd[5] == 'e' ){
		printf("  %d stations trouvées\n", graphe.nb_sommets);
	}
	
	//Permet de lancer le calcul du trajet le plus court 
	//et en fonction de l'argument ecrit différent affichage
 	if(cmd[0] == 'c' && cmd[1] == 'a' && cmd[2] == 'l' && cmd[3] == 'c' && cmd[4] == 'u' && cmd[5] == 'l'){
 		//affichage type application
 		if(cmd[6] == '-' && cmd[7] == 'a'){
			int deb, fin = 0;
		...
		//calcul entre deux stations aléatoire
		if(cmd[6] == '-' && cmd[7] == 'r'){
			srand(time(0));
			int deb, fin = 0;
			deb = rand() % 376;
		...
		//affichage de toutes les stations parcouru
		if(cmd[6] == '-' && cmd[7] == 'l'){
			int deb, fin = 0;
			printf("\tOH SH*T HERE WE GO AGAIN !\n");
			printf("  Veuillez rentrer le numero de la station de départ : ");
		...
		//affichage type application + liste
		if(cmd[6] == '-' && cmd[7] == 't'){
			int deb, fin = 0;
			printf("\tOH SH*T HERE WE GO AGAIN !\n");
			printf("  Veuillez rentrer le numero de la station de départ : ");
		...
		//affichage du trajet entre une stations et toutes les autres
		if(cmd[6] == '-' && cmd[7] == 's'){
			int deb = 0;
			printf("\t****MENU CACHE**** !\n");
			printf("  Veuillez rentrer le numero de la station de départ : ");
			...
			{
				printf("\tStation de départ : %s\n", graphe.station[deb].nom_station);
				printf("\tStation d'arrivee : %s\n\n", graphe.station[i].nom_station);
				graphe = calcul_plus_court_chemin(graphe, deb, i, 2);
			}
